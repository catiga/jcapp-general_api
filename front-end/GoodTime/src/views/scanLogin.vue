        ...
        <div class="login-mid">
            <form autocomplete="off" method="POST" class="form-wrapper" id="login-formWrapper">
                <div class="login-mid-form">
                    <input type="text" v-model="UserPhone" name="username" class="text-input" maxlength="60"
                        placeholder="请输入手机号" value="" id="mobile">
                </div>
                <div class="login-mid-form login-mid-form-code">
                    <input type="number" v-model="ValidateCode" id="img_validate_code" name="verifycode" style="margin-left: 0;"
                        class="text-input-code" value="" placeholder="请输入验证码" autocomplete="off" maxlength="6">
            ...
            godo_login: function () {
                ...
                fetch(url).then(r => r.json()).then(d => {
                    if (d.available) {
                        ...
                        // this.$router.push({name:'scanPurchasingCard'});
                        location.href="/general_api/tcss/index#/purchasingCard/Scanpurchasing"
