	...
	<div><!-- 固定 容器-->
		...
		<div class="my-ticket-box">
			<div class="my-cardWrap">
				<!--影票信息-->
				<div class="my-card my-cardtop my-module-1" style="padding-bottom: 0;min-height: auto;">
					<div class="weui-media weui-media-box_appmsg" v-for="item in OrderList.items" style="margin-left:.3rem; padding: .3rem .3rem .3rem 0;border-bottom: 1px solid #efeff4;">
                        ...
                        <div class="weui-media-box__bd" style="text-align: left;">
                            <div class="select-goods-info">
                                ...
                                <div class="select-goods-info-price" style="margin-top: .1rem;">
                                    <span style="font-size: .24rem;color: #FF6231; font-weight: bold;">¥{{item.unit_amount/100}}</span>
                                </div>
                            </div>
                        </div>
                        <div class="store-goods-cal">
                            <p style="font-size: .28rem;color: #999;">{{item.buy_num}}份</p>
                        </div>

                    </div>

                    <div>
                    	<div class="weui-media-box weui-media-box_text" style="display: inline-block;">
			    ...
			    <!--影票码
			    <div class="my-card my-cardbot my-module-2" style="min-height: 1rem;">
			    	<p class="my-module-2-num">{{movieDetails.seats.length}}张电影票</p>
			    	<p class="my-module-2-hint">在取票机扫码或输入取票码取电影票</p>
			    	<img class="my-module-2-codeimg" :src="'http://qr.liantu.com/api.php?text='+movieDetails.ticket.ticket_flag_1+movieDetails.ticket.ticket_flag_2" />
			    ...
			    <div class="my-card my-cardbot4 my-module-4">
			    	<p class="my-module-5-title" style="border-bottom: 1px solid #efeff4;padding-bottom: .2rem;margin: .3rem .3rem .2rem .3rem;color: #ccc;">订单信息</p>
			    	<p class="my-module-4-nm" style="font-size: .28rem;margin: .4rem .3rem .3rem .3rem;">实付金额<span>{{OrderList.pay_amount/100}}元</span></p>
			    	<p class="my-module-4-info" style="font-size: .28rem;">订单号<span>{{OrderList.order_no}}</span></p>
			    </div>
	...
	export default {
		data : function(){
			...
			showTip:function(msg,ts){
	            ...
	            var url = '/general_api/api/OrderList?apid='+page.apid+'&ds=&token='+token+'&oss=1&ts='+Date.parse(new Date());
	            fetch(url)
	            .then(r => r.json())
	            .then(d => {
	                // console.log(d)
...
.my-module-5:before, .my-module-5:after {
    content: "";
    position: absolute;
    display: block;
    width: .3rem;
