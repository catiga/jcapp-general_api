<template>
	<div style="text-align:center;"><!-- 固定 容器-->
		...
		<div class="purchasingcard-mid">
			<ul class="purchasingcard-mid-ul">
				<li v-if="RechargeData != -1" v-for="(item,index) in RechargeData" :class=" HId == item.id ? 'purchasingcard-mid-li-true' : 'purchasingcard-mid-li' " v-on:click="godo_choose(item.id,index)">
					<p :class=" HId == item.id ? 'purchasingcard-mid-li-grade-true' : 'purchasingcard-mid-li-grade' ">{{item.h_name}}</p>
					<p  :class=" HId == item.id ? 'purchasingcard-mid-li-introduce-true' : 'purchasingcard-mid-li-introduce' ">{{item.cr_type_desc}}</p>
					<p :class=" HId == item.id ? 'purchasingcard-mid-li-price-true' : 'purchasingcard-mid-li-price'">¥{{item.getpay/100}}</p>
		...
		<div class="purchase-box">
			<div class="purchase" v-on:click="godo_recharge">立即开通</div>
		</div>
		<div class="purchasing-num-box" v-if="one">
		...
		<div class="purchasing-num-box" v-if="two">
			...
			SshowNum:function(){
			    ...
			    WeixinJSBridge.invoke(
			      'getBrandWCPayRequest', {
			         ...
			         "package":other.package, //统一下单接口返回的prepay_id参数值
			         "signType":"MD5", //微信签名方式：     
			         "paySign":other.paySign, //微信签名 
			    },
			    function(res){
