...
def ret_data_obj = [];
ret_data_obj.add(ret_result.data);
//返回当前项目的配置信息
...
if(ret_data) {
	...
	if(ret_data['domain']) {
		ret_data['domain'] = JC.request.get().getSchema() + ret_data['domain'];
	}
}
ret_data_obj.add(ret_data);

return GeneralPub.success(ret_data_obj);
